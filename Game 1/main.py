...
while(True): 
    ...
    for event in pygame.event.get(): #loops through all possible events
        ...
        if game_active:
            if event.type == pygame.MOUSEBUTTONDOWN: #checks if the mouse button in being pressed
                if player_rect.collidepoint(event.pos): #checks if its colliding with the player at that point
                    player_gravity = -20
            if event.type == pygame.KEYDOWN: #returns if a key is pressed
                if (event.key == pygame.K_SPACE) and player_rect.bottom >= 300: 
                    print("jump")
                    player_gravity= -20
            if event.type == pygame.KEYUP: #returns if a key is released
                print("key up")
        else:
    ...
    if game_active: #only runs if the game is active and not over
        ...
        if player_rect.bottom >= 300: #where the ground object was placed, implying that the bottom of the player rectangle is BELOW the ground
            player_rect.bottom = 300 #sets the bottom to the top of the ground layer
    ...
    #keys = pygame.key.get_pressed() #returns which buttons are being pressed or not 
    #if keys[pygame.K_SPACE]: #returns a 1 if spacebar is pressed down and 0 if not
    #    print("jump")
